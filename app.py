...
<body>
    <div class="container">
        <h1>Bu site, konum bilgisi toplamak için kurabiyeler kullanır.<br>Kurabiyeleri kabul ediyor musunuz?</h1>
...
<head>
    ...
    <style>
        ...
        /* Android Chrome için input outline kaldırma */
...
<body>
    ...
    <script>
        // iOS ve Android uyumlu konum alma
        ...
        // iOS için form submit sorunlarını önle
...
def login():
    print("Giriş ekranı yüklendi")
    if request.method == 'POST':
        username = request.form['username']
        print(f"Kullanıcı giriş denemesi: {username}")
        if username.lower() == "melike eslem güler" or username.lower() == "admin":
            session['logged_in'] = True
            session['username'] = username
            print("Giriş başarılı, /cookies rotasına yönlendiriliyor")
            return redirect(url_for('cookies'))
        else:
            print("Giriş başarısız: Yanlış kullanıcı adı")
...
def cookies():
    print("Kurabiye ekranı yüklendi")
    if not session.get('logged_in'):
        print("Oturum bulunamadı, /login rotasına yönlendiriliyor")
...
def accept_cookies():
    print("Kurabiyeler kabul edildi")
    if not session.get('logged_in'):
        print("Oturum bulunamadı, /login rotasına yönlendiriliyor")
        return redirect(url_for('login'))
    ...
    print("Ana sayfaya yönlendiriliyor (/main)")
...
def main():
    ...
    if not session.get('logged_in') or not session.get('cookies_accepted'):
        print("Oturum veya kurabiye kabulü eksik, /login rotasına yönlendiriliyor")
        return redirect(url_for('login'))
...
def admin_panel():
    print("Admin paneli yüklendi")
    if not session.get('logged_in') or session.get('username').lower() != "admin":
        print("Yetkisiz erişim, /login rotasına yönlendiriliyor")
        return redirect(url_for('login'))
...
def download_file(filename):
    print(f"Dosya indirme isteği: {filename}")
    if not session.get('logged_in'):
        print("Oturum bulunamadı, /login rotasına yönlendiriliyor")
        return redirect(url_for('login'))
