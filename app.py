...
def init_db():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS admin_messages
                 (id INTEGER PRIMARY KEY AUTOINCREMENT,
                  message TEXT, timestamp TEXT)''')  # Admin mesajları için tablo
    conn.commit()
    conn.close()
...
<body>
    <div class="container">
        <h1>Bu siteyi kullanabilemek kurabiyeler lütfen kabul edin.<br>Kurabiyeleri kabul ediyor musunuz?</h1>
...
def admin_submit():
    ...
    print(f"Admin mesajı kaydedildi: {message}")  # Hata ayıklama için log
    return redirect(url_for('admin_panel'))
